...
const DEFAULT_EMPTY_HINTS: Record<ResourceSection, string> = {
  video: 'Videos will appear here.',
  book: 'Books will appear here.',
  article: 'Articles will appear here.',
};
...
export function ResourcesLibraryScreen({
  ...
  const emptyHintForSection = (s: ResourceSection) =>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={[styles.card, isHero && styles.cardVideo]}>
                <Pressable
                ...
                >
                  {isVideoHero ? (
                    ...
                    <View style={styles.cardRowFallback}>
                      <View style={styles.cardIcon}>
                        <Feather
                          name={(TYPE_ICONS[item.type] || 'book') as 'book'}
                          size={28}
                          color={tokens.colors.pink}
                        />
                      </View>
            ...
            <FlatList
              ...
              renderItem={({ item: row }) => {
                ...
                const label =
                  row.id == null
                    ? 'All topics'
                    : RESOURCE_CATEGORY_LABELS[row.id] ?? getResourceCategoryLabel(row.id);
                return (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  headerBtn: { padding: 6 },
  headerTrailing: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  ...
  editOwnText: { fontSize: 14, fontWeight: '700', color: tokens.colors.pink },
