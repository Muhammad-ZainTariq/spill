...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // URL ?room= (e.g. from Spill app): auto-join that room in Online mode
    const urlParams = new URLSearchParams(window.location.search);
    const urlRoom = urlParams.get('room');
    if (urlRoom && urlRoom.trim()) {
        opponentMode = 'p2p';
        ui.radioP2P.checked = true;
        ui.updateSettingsVisibility();
        ui.p2pPanel.style.display = 'flex';
        ui.netStep1.style.display = 'none';
        ui.netStep2.style.display = 'flex';
        ui.netRoomInput.value = urlRoom.trim();
        netGame.joinRoom(urlRoom.trim());
    } else if (opponentMode === 'p2p') {
